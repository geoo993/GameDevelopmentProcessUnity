...
public class MeshSmoothing: MonoBehaviour {
    ...
    VertexConnection _vertexConnection;
    VertexConnection vertexConnection {
        get {
            if(_vertexConnection == null) {
                _vertexConnection = this.GetComponent<VertexConnection>();
            }
            return _vertexConnection;
        }
        set {
            _vertexConnection = value;
        }
    }
	...
	public Mesh LaplacianFilter (Mesh mesh, int times = 1) {
	...
	private Vector3[] LaplacianFilter(Vector3[] vertices, int[] triangles, int times) {
	...
	private Vector3[] LaplacianFilter(Dictionary<int, VertexConnection> network, Vector3[] origin, int[] triangles) {
	 ...
	 * beta  0.0 ~ 1.0
	*/
	public Mesh HCFilter (Mesh mesh, int times = 5, float alpha = 0.5f, float beta = 0.75f) {
	...
	private Vector3[] HCFilter(Vector3[] vertices, int[] triangles, int times, float alpha, float beta) {
	...
	private Vector3[] HCFilter(Dictionary<int, VertexConnection> network, Vector3[] o, Vector3[] q, int[] triangles, float alpha, float beta) {
